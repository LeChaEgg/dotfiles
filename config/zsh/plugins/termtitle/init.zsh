[[ ${TERM} != dumb ]] && () {
  ...
  local zhooks zhook
  zstyle -a ':dotfiles:termtitle' hooks 'zhooks' || zhooks=(precmd)
  ...
  for zhook in ${zhooks}; do
    if [[ ${TERM_PROGRAM} == Apple_Terminal ]]; then
      ...
      zstyle -s ":dotfiles:termtitle:${zhook}" format 'termtitle_format' || \
          zstyle -s ':dotfiles:termtitle' format 'termtitle_format' || \
