...
if (( ${+_comps} )); then
  print -u2 'warning: completion was already initialized before this module; calling compinit again.'
fi
...
() {
  ...
  local -a zmtimes
  local -i zdump_dat=1
  zstyle -s ':dotfiles:completion' dumpfile 'zdumpfile' || zdumpfile=${ZDOTDIR:-${HOME}}/.zcompdump
...
zstyle -s ':dotfiles:glob' case-sensitivity glob_case_sensitivity || glob_case_sensitivity=insensitive
zstyle -s ':dotfiles:completion' case-sensitivity completion_case_sensitivity || completion_case_sensitivity=insensitive
